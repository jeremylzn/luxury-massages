...
export class AuthService implements OnDestroy {
  ...
  logout() {
    ...
    let role = JSON.parse(localStorage.getItem(this.authLocalStorageToken)).user.role
    localStorage.removeItem(this.authLocalStorageToken);
    (role == 2) ? this.router.navigate(['/auth/worker/login'], {queryParams: {}}) :
  ...
  clearLocalStorage() {
    ...
    if (localStorage.getItem(this.authLocalStorageToken))
      role = JSON.parse(localStorage.getItem(this.authLocalStorageToken)).user.role
    localStorage.removeItem(this.authLocalStorageToken);
    (role == 2) ? this.router.navigate(['/auth/worker/login'], {queryParams: {}}) :
    this.router.navigate(['/auth/login'], {queryParams: {}});
  }
  ...
  // private handleAuth(email: string, id: string, token: string, fullname: string, telephone: string, address: AddressModel) {
  //   const user = new BehaviorSubject<UserModel>({email, id, fullname, telephone, address, accessToken})
  //   this.token = token;
